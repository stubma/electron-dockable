...
class DockTab extends window.HTMLElement {
	...
	_onClose(event) {
		...
		// close panel
		let index = this.tabBar.indexOf(this);
		this.dockNode.closePanelAt(index, true);
	}
	...
	_onDragStart(event) {
		...
		// save drag data
		event.dataTransfer.setData("edock/type", "tab");
		DockUtils.dragType = "tab";
		DockUtils.dropSide = -1;
		DockUtils.draggedTab = this;
	}
